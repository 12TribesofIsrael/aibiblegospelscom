...
- **Ad tracking + first-touch attribution — this site had none (2026-07-30).** It carried only Vercel Analytics, which counts visits but cannot tell you that a TikTok clip produced an email signup — and TikTok is this brand's largest traffic source. New [src/components/Tracking.tsx](src/components/Tracking.tsx) loads **GA4 + Meta Pixel + TikTok Pixel** site-wide from `NEXT_PUBLIC_GA4_MEASUREMENT_ID` / `NEXT_PUBLIC_META_PIXEL_ID` / `NEXT_PUBLIC_TIKTOK_PIXEL_ID`; each block renders nothing until its id is set, so the site ships tracking-ready and dark (a **redeploy** is required after setting one — `NEXT_PUBLIC_*` is baked at build time). Named `Tracking`, not `Analytics`, because `layout.tsx` already renders Vercel's own `<Analytics/>` — they coexist. New [src/lib/pixel.ts](src/lib/pixel.ts) exposes `trackLead`, now fired on the success path of **both** capture forms and tagged by magnet (`deut28` vs `study-guide`) so the two offers separate *inside one pixel* via `content_name` rather than being split across two pixels that would each be starved of the conversion volume Meta needs to leave its learning phase.
- **First-touch UTM capture, and the leak it closes.** [src/lib/attribution.ts](src/lib/attribution.ts) stashes `utm_*` / `gclid` / `fbclid` / referrer into a 30-day `abg_attr` cookie on the first page of a visit, and [src/lib/attribution-server.ts](src/lib/attribution-server.ts) reads it back off the request. This fixes a real leak: `/api/subscribe` was sending the CRM `source: "website-form"` and **nothing else**, so every lead from this site landed permanently unattributable — `pushLeadToCrm` already accepted an `attribution` field and nobody was passing one. The cookie is read while the request is still in scope, deliberately *before* `after()` runs, so the post-response callback never reaches for a `Request` that has gone away. Ported from the agency site so both brands speak the same attribution shape; cookie name differs only to make the capturing origin obvious when debugging.
