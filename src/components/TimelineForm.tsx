...
/**
 * Reading-plan capture for The Complete Biblical Timeline page.
 *
 * Posts the same /api/subscribe route as the other forms, but with
 * `magnet: "timeline"` so it gets its own email and its own CRM hosted form
 * (whose autoTags are the only attribution signal — see lib/crm.ts).
 */
export default function TimelineForm({ source = "timeline-page" }: { source?: string }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/subscribe", {
        ...
        body: JSON.stringify({ email, source, magnet: "timeline" }),
      });
  ...
  return (
    <div className="max-w-xl mx-auto rounded-2xl border border-brand-border bg-brand-black/40 p-8 text-center">
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col sm:flex-row gap-3">
        <label htmlFor="timeline-email" className="sr-only">
          Email address
        </label>
        <input
          id="timeline-email"
